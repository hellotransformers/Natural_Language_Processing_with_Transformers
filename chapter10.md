...
随着数据集越来越大，你能完全控制--或者至少对里面的内容有一个准确的概念--的机会就会减少。一个非常大的数据集很可能不是由专门的创造者组装的，他们一次只制作一个例子，同时意识到并了解整个管道和机器学习模型将被应用的任务。相反，更有可能的是，一个非常大的数据集是通过收集其他活动的副作用产生的数据，以自动或半自动的方式创建的。例如，它可能由一个公司存储的所有文件（如合同、采购订单等）、用户活动的日志或从互联网上收集的数据组成。
...
让我们通过比较GPT和GPT-2的文本生成，来说明模型被数据所歪曲的概念。GPT主要是在BookCorpus上训练的，而GPT-2是在网页、博客和从Reddit链接的新闻文章上训练的。我们将在同一提示上比较两个模型的相似大小的版本，因此主要的区别是预训练数据集，我们将使用文本生成管道来研究模型的输出:
...
一般来说，任何在数据集上训练的模型都会反映出其训练数据中的语言偏差以及人口和事件的过度或不足。对于与模型互动的目标受众来说，模型行为中的这些偏差是需要考虑的；对于一些有用的指南，我们推荐你参考谷歌的一篇论文，该论文提供了一个数据集开发的框架。
...
现在我们已经收集并加载了我们的大数据集，让我们来看看如何有效地处理数据以反馈给我们的模型。在以前的章节中，我们使用了伴随我们使用的模型的标记器。这是有道理的，因为这些模型是使用通过标记器中定义的特定预处理管道的数据进行预训练的。当使用预训练的模型时，坚持使用为预训练选择的相同的预处理设计是很重要的。否则，该模型可能会被送入分布错误的模式或未知的标记。
...
正如你在第四章所看到的，标记器是一个由四个步骤组成的处理管道：规范化、预标记化、标记器模型和后处理。标记器管道中可以在数据上进行训练的部分是标记器模型。正如我们在第二章所讨论的，有几种子词标记化算法可以使用，如BPE、WordPiece和Unigram。
...
这是一个相当奇怪的输出，所以让我们试着通过运行标记器管道的各个子模块来了解这里发生了什么。首先，让我们看看在这个标记器中应用了什么规范化：
...
现在我们可以使用生成管道从一个给定的提示中生成候选的完成度。默认情况下，该流水线将生成代码，直到预定义的最大长度，而且输出可能包含多个函数或类。因此，为了保持输出的简洁，我们将实现一个first_block()函数，使用正则表达式来提取一个函数或类的第一次出现。下面的complete_code()函数应用这个逻辑来打印出由CodeParrot生成的补全内容：
